...
def barras_rec(conn,tabla):
    ...
    plot.savefig("GraficoBarras_recuperacion.png", bbox_inches='tight')
...
def cicular_atencion(conn,tabla):
   ...
   ax.pie(datos['total'], labels=datos['ubicacion_caso'], autopct='%1.1f%%',startangle=90)
   ax.axis('equal')
...
def dosd_contagios(conn,tabla):
    ...
    plot.savefig("Grafico_Depto.png", bbox_inches='tight')
...
def fechas(conn, tabla):
    ...
    plot.savefig("Graficocontagios.png", bbox_inches='tight')
...
def fechasmuerte(conn, tabla):
    ...
    loc = ticker.MultipleLocator(base=20)
    ax.xaxis.set_major_locator(loc)
    plot.savefig("Graficomuerte.png", bbox_inches='tight')